...
- **暗色 / 亮色主题**：一键切换暗色主题（设置对话框或配置文件），长时间使用不刺眼
- **设置对话框**：集中调整主题、注入等待超时（3–120 秒）、日志最大行数（50–10000 行），即时生效
...
| --- | --- |
| 主题 | 亮色 / 暗色，切换后即时生效并记住 |
| 注入超时(秒) | 启动注入时等待 DLL 初始化的超时时间（3–120 秒，默认 15） |
| 日志最大行数 | 界面日志保留的最大行数（50–10000 行，默认 1000） |
...
CreateRemoteThread / NtCreateThreadEx / QueueUserAPC(LoadLibraryW)
      ...
      ▼
WaitForSingleObject(按设置超时，默认 15s)（APC 方式跳过等待，改为恢复后核验）
      │  等待 DLL 初始化完成
      ▼
ResumeThread
      │  恢复目标进程主线程，程序正常运行
